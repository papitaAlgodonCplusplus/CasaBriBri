...
const visualObjects = [
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('11%'), y: hp('0%') } : getResponsivePos(16, 1),
    size: {
  ...
  },
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('54%'), y: hp('24%') } : getResponsivePos(115, 50),
    size: {
  ...
  },
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('4%'), y: hp('5.5%') } : getResponsivePos(1, 10),
    size: {
  ...
  },
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('33.5%'), y: hp('52%') } : getResponsivePos(67, 104),
    size: {
  ...
  },
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('27%'), y: hp('0%') } : getResponsivePos(53, 0),
    size: {
  ...
  },
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('55%'), y: hp('1%') } : getResponsivePos(115.5, 1),
    size: {
      normal: { width: wp('21%'), height: hp('28%') },
      selected: { width: wp('21%'), height: hp('28%') },
    },
  ...
  },
  {
    ...
    position: Platform.OS === 'web' ? { x: wp('38%'), y: hp('-3%') } : getResponsivePos(78, -6),
    size: {
      normal: { width: wp('20%'), height: hp('26%') },
      selected: { width: wp('21%'), height: hp('28%') },
    },
...
const draggableElements = [
  { id: 1, name: 'iwo', image: require('@/assets/images/iwo2.png') },
  { id: 2, name: 'kapokwa', image: require('@/assets/images/kapokwa2.png') },
  { id: 3, name: 'ak_wawe', image: require('@/assets/images/ak_wawe2.png') },
  { id: 4, name: 'u_shu', image: require('@/assets/images/u_shu2.png') },
  { id: 5, name: 'ulok', image: require('@/assets/images/ulok2.png') },
  { id: 6, name: 'ko', image: require('@/assets/images/ko2.png') },
  { id: 7, name: 'u_kko', image: require('@/assets/images/u_kko2.png') },
];

const wordColors = [
  { name: 'iwo', color: '#e4191c' },
  { name: 'kapokwa', color: '#ede430' },
  { name: 'ak_wawe', color: '#0046e3' },
  { name: 'u_shu', color: '#603f91' },
  { name: 'ulok', color: '#68e033' },
  { name: 'ko', color: '#e6175c' },
  { name: 'u_kko', color: '#e94d1f' },
];
  ...
  const handleWordPress = (item: { name: string }) => {
    ...
    if (selectedObject) {
      ...
      if (objectInfo && objectInfo.correctWord === item.name) {
        setMatches(prev => ({ ...prev, [selectedObject]: item.name }));
  ...
  const handleObjectPress = (objectName: string) => {
    ...
    if (selectedWord) {
      ...
      if (objectInfo && objectInfo.correctWord === selectedWord) {
        setMatches(prev => ({ ...prev, [objectName]: selectedWord }));
        stopPulseAnimation(objectName);
        setSelectedObject(null);
        setSelectedWord(null);
      }
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          {visualObjects.map(item => (
            ...
            >
              <Animated.View style={{ transform: [{ scale: animatedValues[item.name] }] }}>
                <Image
                  source={selectedObject === item.name || matches[item.name] ? item.imageSelected : item.imageNormal}
                  style={{
                    width:
